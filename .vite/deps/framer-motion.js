...
function updateMotionValuesFromProps(element, next, prev) {
  for (const key in next) {
    ...
    if (isMotionValue(nextValue)) {
      ...
      if (true) {
        warnOnce(nextValue.version === "11.3.20", `Attempting to mix Framer Motion versions ${nextValue.version} with 11.3.20 may not work as expected.`);
      }
